...
import sys
import os
from dataset.data import ContrastiveSegDataset
from dataset.utils import *
from torchvision import transforms as T



@hydra.main(config_path="configs", config_name="train_config.yaml") # reading config file
def train(cfg: DictConfig):
    OmegaConf.set_struct(cfg, False) # init config
    ...
    geometric_transforms = T.Compose([ # transforms mostly used in self-supervised learning
        T.RandomHorizontalFlip(),
        T.RandomResizedCrop(size=cfg.res, scale=(0.8, 1.0))
    ])
    photometric_transforms = T.Compose([ # transforms mostly used in self-supervised learning
